...
func run_command(command: String, cwd: String = "", context: String = "") -> void:
...
func reject_changes(cwd: String = "") -> void:
	var body := {}
	if not cwd.is_empty():
		body["cwd"] = cwd
	_do_post("/reject", body)


func apply_proposed(files: Array, cwd: String = "") -> void:
